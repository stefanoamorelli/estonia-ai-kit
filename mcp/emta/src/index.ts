...
class EMTAMCPServer {
      ...
      try {
        switch (name) {
          ...
          case 'get_tax_residency':
            return await this.getTaxResidency(args.personalCode as string, args.year as number);
          ...
          case 'get_vat_returns':
            return await this.getVATReturns(args.registryCode as string, args.year as number);

          case 'get_social_tax_info':
            return await this.getSocialTaxInfo(args.registryCode as string, args.period as string);
            ...
            text: JSON.stringify(
              {
                valid: false,
                error:
                  'Invalid VAT number format. Estonian VAT numbers must start with EE followed by 9 digits.',
              },
              null,
              2
            ),
  ...
  private async getEmploymentRegistrations(
    registryCode: string,
    startDate?: string,
    endDate?: string
  ) {
  ...
  private async getVATReturns(registryCode: string, year: number) {
    ...
    for (let month = 1; month <= 12; month++) {
      mockData.push({
        ...
        submitted: month < 11,
        submissionDate:
          month < 11 ? `${year}-${(month + 1).toString().padStart(2, '0')}-20` : undefined,
